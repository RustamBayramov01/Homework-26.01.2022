...
namespace ConsoleApp1
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // var list1 = debtors.Where(s => s.Email.Contains("jourrapide.com"));
            // var list2 = debtors.Where(s => s.Email.Contains("rhyta.com"));
            // foreach (var debtor in list1) Console.WriteLine(debtor.Email);
            // foreach (var debtor in list2) Console.WriteLine(debtor.Email);




            // 2) Yashi 26-dan 36-ya qeder olan borclulari cixartmag
            // var result = debtors.Where(s => s.BirthDay.Year >= 1986).ToList();
            ...
            // var result = debtors.Where(s => s.Debt < 5000).ToList();
            // result.ForEach(s => Console.WriteLine(s));
            ...
            // var result = debtors.Where(s => s.FullName.Length > 18 && s.Phone.Contains("7")).ToList();
            // result.ForEach(s => Console.WriteLine(s));
            ...
            // var result = debtors.Where(s => s.BirthDay.Month == 12).ToList();
            // result.ForEach(s => Console.WriteLine(s.FullName + "\t\t" + s.BirthDay + "\t\t" + s.Debt));
            ...
            // var result = debtors.Where(s => s.BirthDay.Month == 12).OrderBy(d => d.FullName).ToList();
            // foreach (var debtor in result) Console.WriteLine(debtor.FullName + "\t\t" + debtor.Debt);



            // 7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
            // var list = debtors.Where(s => !s.Phone.Contains("8")).ToList();
            // foreach (var debtor in list) Console.WriteLine(debtor.FullName + " " + (2022 - debtor.BirthDay.Year) + " " + debtor.Debt);
            ...
            // var list = debtors.FindAll(d => d.FullName.Any(c => d.FullName.Split(c).Length > 3)).OrderBy(d => d.FullName).ToList();
            // foreach (var debtor in list) Console.WriteLine(debtor.FullName);
            ...
            //int allDebt = 0;
            //var result = debtors.Where(s => s.Debt > 0).ToList();
            //
            //foreach (var item in result) allDebt += item.Debt;
            //
            //Console.WriteLine($"All Debt : {allDebt}");





            // 11) 2-ci dunya muharibesin gormush borclularin siyahisi cixartmaq
            // var list = debtors.Where(s => s.BirthDay.Year <= 1939).ToList();
            ...
            // var list = debtors.FindAll(d => d.Phone.Any(c => d.Phone.Split(c).Length < 2)).OrderBy(d => d.Phone).ToList();
            // foreach (var debtor in list) Console.WriteLine(debtor.Phone);
            ...
            // var list = debtors.Where(s => (s.Debt / (12 - s.BirthDay.Month)) <= 500).ToList();
            // foreach (var item in list) Console.WriteLine(item.BirthDay.Month);
        }
    }
}